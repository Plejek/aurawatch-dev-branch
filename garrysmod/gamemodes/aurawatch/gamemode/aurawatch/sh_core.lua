...
openAura.Timers = {};
openAura.DataStreamHooks = {};
openAura.GlobalSharedVars = {};
...
function openAura:URLEncode(url)
	local output = "";
...
function openAura:GetLogTypeColor(logType)
	local logTypes = {
		Color(255, 50, 50, 255),
		Color(255, 150, 0, 255),
		Color(255, 200, 0, 255),
...
function openAura:GetCoreVersion()
	return self.CoreVersion;
end;

-- A function to get the auth version.
function openAura:GetAuthVersion()
	return self.AuthVersion;
end;

-- A function to get the schema folder.
function openAura:GetSchemaFolder()
...
function openAura:GetPluginDirectory()
	return PLUGIN_DIRECTORY;
end;

-- A function to get the OpenAura folder.
function openAura:GetOpenAuraFolder()
	local folder = string.gsub(self.OpenAuraFolder, "gamemodes/", "");
...
function openAura:SplitString(text, interval)
...
function openAura:GetAmmoInformation(weapon)
...
function openAura:HandlePlayerJumping(player)
...
function openAura:HandlePlayerDucking(player, velocity)
	if ( player:Crouching() ) then
		local model = player:GetModel();
		local weapon = player:GetActiveWeapon();
		local raised = self.player:GetWeaponRaised(player, true);
...
function openAura:HandlePlayerSwimming(player)
	if (player:WaterLevel() >= 2) then
...
function openAura:HandlePlayerDriving(player)
...
function openAura:UpdateAnimation(player, velocity, maxSeqGroundSpeed)
	local velLength = velocity:Length2D();
	local rate = 1.0;
...
function openAura:CalcMainActivity(player, velocity)
	local model = player:GetModel();
...
function openAura:TranslateActivity(player, act)
	local model = player:GetModel();
	local raised = self.player:GetWeaponRaised(player, true);
...
function openAura:DoAnimationEvent(player, event, data)
	local model = player:GetModel();
...
if (SERVER) then
	openAura.Entities = {};
	openAura.TempPlayerData = {};
	openAura.HitGroupBonesCache = {
	...
	function openAura:LoadBans()
		self.BanList = self:RestoreOpenAuraData("bans");
		...
		self:SaveOpenAuraData("bans", self.BanList);
	end;
	
	-- A function to add a ban.
	function openAura:AddBan(identifier, duration, reason, Callback, saveless)
		local steamName = nil;
	...
	function openAura:RemoveBan(identifier, saveless)
	...
	function openAura:StartDataStream(player, name, data)
	...
	function openAura:SaveSchemaData(fileName, data)
		fileio.Write( self:SetupFullDirectory("settings/openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".aura"), glon.encode(data) );
	end;

	-- A function to delete schema data.
	function openAura:DeleteSchemaData(fileName)
		fileio.Delete( self:SetupFullDirectory("settings/openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".aura") );
	end;

	-- A function to check if schema data exists.
	function openAura:SchemaDataExists(fileName)
		return _file.Exists("settings/openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".aura", true);
	end;

	-- A function to restore schema data.
	function openAura:RestoreSchemaData(fileName, default)
		if ( self:SchemaDataExists(fileName) ) then
			local data = fileio.Read( self:SetupFullDirectory("settings/openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".aura") );
	...
	-- A function to restore OpenAura data.
	function openAura:RestoreOpenAuraData(fileName, default)
		if ( self:OpenAuraDataExists(fileName) ) then
			local data = fileio.Read( self:SetupFullDirectory("settings/openaura/"..fileName..".aura") );
	...
	function openAura:SetupFullDirectory(filePath)
		local directory = string.gsub(relative.Get()..filePath, "\\", "/");
	...
	-- A function to save OpenAura data.
	function openAura:SaveOpenAuraData(fileName, data)
		fileio.Write( self:SetupFullDirectory("settings/openaura/"..fileName..".aura"), glon.encode(data) );
	end;

	-- A function to check if OpenAura data exists.
	function openAura:OpenAuraDataExists(fileName)
		return _file.Exists("settings/openaura/"..fileName..".aura", true);
	end;

	-- A function to delete OpenAura data.
	function openAura:DeleteOpenAuraData(fileName)
		fileio.Delete( self:SetupFullDirectory("settings/openaura/"..fileName..".aura") );
	end;

	-- A function to convert a force.
	function openAura:ConvertForce(force, limit)
	...
	function openAura:SavePlayerAttributeBoosts(player, data)
	...
	function openAura:CalculateSpawnTime(player, inflictor, attacker, damageInfo)
		local info = {
			attacker = attacker,
			inflictor = inflictor,
	...
	function openAura:CreateDecal(texture, position, temporary)
	...
	function openAura:HandleWeaponFireDelay(player, raised, weapon, curTime)
		local delaySecondaryFire = nil;
		local delayPrimaryFire = nil;
		
		if ( !self.plugin:Call("PlayerCanFireWeapon", player, raised, weapon, true) ) then
			delaySecondaryFire = curTime + 60;
		end;
		
		if ( !self.plugin:Call("PlayerCanFireWeapon", player, raised, weapon) ) then
	...
	function openAura:ScaleDamageByHitGroup(player, attacker, hitGroup, damageInfo, baseDamage)
	...
	function openAura:CalculatePlayerDamage(player, hitGroup, damageInfo)
	...
	function openAura:GetRagdollHitBone(entity, position, default, minimum)
		local closest = {};
	...
	function openAura:GetRagdollHitGroup(entity, position)
	...
	function openAura:CreateBloodEffects(position, decals, entity, force)
		if (!force) then
			force = VectorRand() * 80;
		end;
		...
		util.Effect("aura_bloodsmoke", effectData, true, true);
	...
	function openAura:DoEntityTakeDamageHook(gamemode, arguments)
	...
	function openAura:PerformDateTimeThink()
	...
	function openAura:CreateConVar(name, value, flags, Callback)
		...
		cvars.AddChangeCallback(name, function(conVar, previousValue, newValue)
			self.plugin:Call("OpenAuraConVarChanged", conVar, previousValue, newValue);
	...
	function openAura:IsShuttingDown()
		return self.ShuttingDown;
	end;
	
	-- A function to distribute wages cash.
	function openAura:DistributeWagesCash()
		for k, v in ipairs( _player.GetAll() ) do
	...
	function openAura:IncludeSchema()
	...
	function openAura:PrintLog(logType, text)
		local recipientFilter = RecipientFilter();
		
		for k, v in ipairs( _player.GetAll() ) do
			if (v:HasInitialized() and v:GetInfoNum("aura_showlog", 0) == 1) then
		...
		if ( AURA_CONVAR_LOG:GetInt() == 1 and isDedicatedServer() ) then
			self:ServerLog(text);
		end;
	end;
	
	-- A function to log to the server.
	function openAura:ServerLog(text)
		ServerLog(text.."\n");
	...
	openAura.ProgressBarColor = Color(50, 100, 150, 200);
	openAura.TargetPlayerText = { text = {} };
	openAura.BackgroundBlurs = {};
	openAura.RecognisedNames = {};
	openAura.PlayerInfoText = { text = {}, width = 0, subText = {} };
	openAura.NetworkProxies = {};
	openAura.InfoMenuOpen = false;
	openAura.ColorModify = {};
	openAura.Cinematics = {};
	openAura.MenuItems = { items = {} };
	openAura.ESPInfo = {};
	openAura.Hints = {};
	openAura.Bars = { x = 0, y = 0, width = 0, height = 0, bars = {} };

	-- A function to register a network proxy.
	function openAura:RegisterNetworkProxy(entity, name, Callback)
	...
	function openAura:IsInfoMenuOpen()
		return self.InfoMenuOpen;
	end;
	
	-- A function to get some a menu item.
	function openAura.MenuItems:Get(text)
	...
	function openAura.MenuItems:Add(text, panel, tip)
		self.items[#self.items + 1] = {text = text, panel = panel, tip = tip};
	end;

	-- A function to destroy a menu item.
	function openAura.MenuItems:Destroy(text)
		for k, v in pairs(self.items) do
			if (v.text == text) then
	...
	function openAura.TargetPlayerText:Add(uniqueID, text, color)
	...
	function openAura.TargetPlayerText:Get(uniqueID)
	...
	function openAura.TargetPlayerText:Destroy(uniqueID)
		for k, v in pairs(self.text) do
			if (v.uniqueID == uniqueID) then
	...
	function openAura.PlayerInfoText:DoesAnyExist()
		return (#self.text > 0 or #self.subText > 0);
	end;

	-- A function to add some player info text.
	function openAura.PlayerInfoText:Add(uniqueID, text)
		if (text) then
			self.text[#self.text + 1] = {
				uniqueID = uniqueID,
				text = text
	...
	function openAura.PlayerInfoText:Get(uniqueID)
		for k, v in pairs(self.text) do
			if (v.uniqueID == uniqueID) then
				return v;
			end;
	...
	function openAura.PlayerInfoText:AddSub(uniqueID, text, priority)
		if (text) then
	...
	function openAura.PlayerInfoText:GetSub(uniqueID)
	...
	function openAura.PlayerInfoText:Destroy(uniqueID)
	...
	function openAura.PlayerInfoText:DestroySub(uniqueID)
	...
	function openAura.Bars:Get(uniqueID)
	...
	function openAura.Bars:Add(uniqueID, color, text, value, maximum, flash, priority)
	...
	function openAura.Bars:Destroy(uniqueID)
		for k, v in pairs(self.bars) do
			if (v.uniqueID == uniqueID) then
	...
	function openAura:CreateClientConVar(name, value, save, userData, Callback)
	...
	function openAura:WrapText(text, font, width, baseTable)
	...
	function openAura:HandleEntityMenu(entity)
		local options = {};
		local menu = nil;
	...
	function openAura:GetGradientTexture()
		return self.GradientTexture;
	end;
	
	-- A function to add a menu from data.
	function openAura:AddMenuFromData(menu, data, Callback)
		local options = {};
		local created;
	...
	function openAura:AdjustMaximumWidth(font, text, width, addition, extra)
		local textString = tostring( openAura:Replace(text, "&", "U") );
	...
	function openAura:AddTopHint(text, delay, color, noSound)
	...
	function openAura:CalculateHints()
	...
	function openAura:DrawDateTime()
		...
		if ( self.plugin:Call("PlayerCanSeeDateTime") ) then
			...
			local textWidth, textHeight = openAura:GetCachedTextSize(dateTimeFont, text);
		...
		self:DrawBars(info, "tab");
			self.plugin:Call("OpenAuraDateTimeDrawn", info);
		...
		if ( self.plugin:Call("PlayerCanSeeLimbDamage") ) then
			...
			openAura.plugin:Call("GetPlayerLimbInfo", texInfo);
	...
	function openAura:DrawBars(info, class)
	...
	function openAura:GetESPInfo()
		return self.ESPInfo;
	end;
	
	-- A function to draw the admin ESP.
	function openAura:DrawAdminESP()
		local colorWhite = self.option:GetColor("white");
		local curTime = UnPredictedCurTime();
	...
	function openAura:DrawBar(x, y, width, height, color, text, value, maximum, flash, barInfo)
		local backgroundColor = self.option:GetColor("background");
	...
	function openAura:GetScreenCenter()
		return ScrW() / 2, (ScrH() / 2) + 32;
	end;
	
	-- A function to draw some simple text.
	function openAura:DrawSimpleText(text, x, y, color, alignX, alignY, shadowless, shadowDepth)
		local mainTextFont = self.option:GetFont("main_text");
	...
	--]]
	function openAura:PrintColoredText(...)
		...
		chat.OpenAuraAddText( unpack(text) );
	end;
	
	-- A function to get whether a custom crosshair is used.
	function openAura:UsingCustomCrosshair()
	...
	function openAura:DrawInfo(text, x, y, color, alpha, alignLeft, Callback, shadowDepth)
		local mainTextFont = self.option:GetFont("main_text");
	...
	function openAura:CanCreateInfoMenuPanel()
	...
	function openAura:CreateInfoMenuPanel(x, y, w)
		if ( !IsValid(self.InfoMenuPanel) ) then
			local options = {};
	...
	function openAura:GetRagdollEyeAngles()
	...
	function openAura:DrawSimpleGradientBox(cornerSize, x, y, width, height, color, alphaMax, bNoBox)
	...
	function openAura:DrawTexturedGradientBox(cornerSize, x, y, width, height, color, alphaMax, bNoBox)
	...
	-- Adding some backwards compatability here.
	openAura.DrawRoundedGradient = openAura.DrawTexturedGradientBox;

	-- A function to draw a player information sub box.
	function openAura:DrawPlayerInfoSubBox(text, x, y, width, boxInfo)
		local backgroundColor = self.option:GetColor("background");
		local colorInfo = self.option:GetColor("information");
	...
	function openAura:CreateColoredSpawnIcon(parent)
	...
	function openAura:DrawArmorBar()
	...
	function openAura:DrawHealthBar()
	...
	function openAura:GetNoticePanel()
	...
	function openAura:SetNoticePanel(noticePanel)
		self.NoticePanel = noticePanel;
	end;
	
	-- A function to add some cinematic text.
	function openAura:AddCinematicText(text, color, barLength, hangTime, font, bThisOnly)
		local colorWhite = self.option:GetColor("white");
	...
	function openAura:IsUsingCamera()
		if (IsValid( self.Client:GetActiveWeapon() )
	...
	function openAura:GetTargetIDData()
		return self.TargetIDData;
	end;
	
	-- A function to calculate the screen fading.
	function openAura:CalculateScreenFading()
	...
	function openAura:DrawCinematic(cinematicTable, curTime)
	...
	function openAura:DrawCinematicIntro(curTime)
	...
	function openAura:DrawCinematicIntroBars()
	...
	function openAura:DrawCinematicInfo()
	...
	function openAura:DrawDoorText(entity, eyePos, eyeAngles, font, nameColor, textColor)
	...
	function openAura:IsCharacterScreenOpen(isVisible)
	...
	-- A function to save schema data.
	function openAura:SaveSchemaData(fileName, data)
		_file.Write( "openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".txt", glon.encode(data) );
	end;

	-- A function to delete schema data.
	function openAura:DeleteSchemaData(fileName)
		_file.Delete("openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".txt");
	end;

	-- A function to check if schema data exists.
	function openAura:SchemaDataExists(fileName)
		return _file.Exists("openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".txt");
	end;

	-- A function to restore schema data.
	function openAura:RestoreSchemaData(fileName, default)
		if ( self:SchemaDataExists(fileName) ) then
			local data = _file.Read("openaura/schemas/"..self:GetSchemaFolder().."/"..fileName..".txt");
	...
	function openAura:RestoreOpenAuraData(fileName, default)
		if ( self:OpenAuraDataExists(fileName) ) then
			local data = _file.Read("openaura/"..fileName..".txt");
	...
	-- A function to save OpenAura data.
	function openAura:SaveOpenAuraData(fileName, data)
		_file.Write( "openaura/"..fileName..".txt", glon.encode(data) );
	end;

	-- A function to check if OpenAura data exists.
	function openAura:OpenAuraDataExists(fileName)
		return _file.Exists("openaura/"..fileName..".txt");
	end;

	-- A function to delete OpenAura data.
	function openAura:DeleteOpenAuraData(fileName)
		_file.Delete("openaura/"..fileName..".txt");
	end;
	
	-- A function to run a OpenAura command.
	function openAura:RunCommand(command, ...)
		RunConsoleCommand("aura", command, ...);
	end;
	
	-- A function to get whether the local player is choosing a character.
	function openAura:IsChoosingCharacter()
	...
	-- A function to start a data stream.
	function openAura:StartDataStream(name, data)
		local encodedData = glon.encode(data);
...
function openAura:ExplodeByTags(text, seperator, open, close, hide)
	local results = {};
	local current = "";
	local tag = nil;
...
--]]
function openAura:ExplodeString(seperator, text)
...
function openAura:SetCamelCase(text, camelCase)
	if (camelCase) then
...
function openAura:IncludeDirectory(directory)
	if (string.sub(directory, -1) != "/") then
		directory = directory.."/";
	end;
	
	for k, v in pairs( _file.FindInLua(directory.."*.lua") ) do
		openAura:IncludePrefixed(directory..v);
	end;
end;
...
function openAura:IncludePlugins(directory)
	if (string.sub(directory, -1) != "/") then
		directory = directory.."/";
	end;
	...
	for k, v in pairs( _file.FindInLua(directory.."*") ) do
		if (v != ".." and v != ".") then
			if (CLIENT) then
				if ( _file.IsDir("lua_temp/"..directory..v, true) ) then
					self.plugin:Include(directory..v);
				end;
			elseif ( _file.IsDir("gamemodes/"..directory..v, true) ) then
				self.plugin:Include(directory..v);
			end;
		end;
	end;
...
function openAura:CallTimerThink(curTime)
	for k, v in pairs(self.Timers) do
		if (!v.paused) then
			if (curTime >= v.nextCall) then
				...
				if (!success) then
					ErrorNoHalt("OpenAura -> the "..tostring(k).." timer has failed to run.");
					ErrorNoHalt(value);
				end;
...
function openAura:TimerExists(name)
	return self.Timers[name];
end;

-- A function to start a timer.
function openAura:StartTimer(name)
...
function openAura:PauseTimer(name)
	if (self.Timers[name] and !self.Timers[name].paused) then
...
function openAura:DestroyTimer(name)
	self.Timers[name] = nil;
end;

-- A function to create a timer.
function openAura:CreateTimer(name, delay, repetitions, Callback, ...)
	self.Timers[name] = {
		calls = 0,
		delay = delay,
...
function openAura:HasObjectAccess(player, object)
...
function openAura:DeriveFromSandbox()
	...
	if (!self.IsSandboxDerived) then
		...
		if (sandboxGamemode and baseGamemode) then
			...
			table.Inherit(openAura, sandboxGamemode);
...
function openAura:GetSortedCommands()
	local commands = {};
...
function openAura:ZeroNumberToDigits(number, digits)
...
function openAura:GetShortCRC(value)
	return math.ceil(util.CRC(value) / 100000);
end;

-- A function to validate a table's keys.
function openAura:ValidateTableKeys(base)
	for i = 1, #base do
		if ( !base[i] ) then
			table.remove(base, i);
		end;
...
function openAura:GetPhysicsEntities()
	local entities = {};
...
function openAura:CreateMulticallTable(base, object)
...
function openAura:SharedVarsHaveInitialized()
...
function openAura:ConvertUserMessageClass(class)
	local convertTable = {
...
function openAura:GetDefaultNetworkedValue(class)
...
function openAura:ConvertNetworkedClass(class)
	return NETWORKED_CLASS_TABLE[class];
end;

-- A function to get the default class value.
function openAura:GetDefaultClassValue(class)
	local convertTable = {
		["String"] = "",
		["Entity"] = NULL,
		["Vector"] = Vector(0, 0, 0),
...
function openAura:SetSharedVar(key, value)
...
function openAura:GetSharedVar(key)
	local entity = GetWorldEntity();
...
function openAura:RegisterGlobalSharedVar(name, class)
	self.GlobalSharedVars[#self.GlobalSharedVars + 1] = {name, class};
end;

-- A function to create fake damage info.
function openAura:FakeDamageInfo(damage, inflictor, attacker, position, damageType, damageForce)
...
function openAura:UnpackColor(color)
	...
	if (CLIENT) then
		for k in string.gmatch(text, ":(.-):") do
			if ( k != "" and input.LookupBinding(k) ) then
				text = openAura:Replace(text, ":"..k..":", "<"..string.upper( tostring( input.LookupBinding(k) ) )..">");
			end;
		end;
	end;
